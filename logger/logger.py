...
class Logger:
    ...
    def calc_accuracy(self, output, labels, from_logits=False):
        if from_logits:
            ...
            assert all(output >= 0) and all(
                output <= 1
            ), "output should be in [0,1], if not from logits. Did you forget to apply sigmoid or not specify from_logits=True?"
            predictions = torch.round(output)
        assert (
            predictions.shape == labels.shape
        ), f"Predictions shape {predictions.shape} does not match labels shape {labels.shape}, {predictions}, {labels}"
    ...
    def log_accuracy(
        ...
        fake_accuracy = self.calc_accuracy(
            output[labels == 0], labels[labels == 0], from_logits
        )
        real_accuracy = self.calc_accuracy(
            output[labels == 1], labels[labels == 1], from_logits
        )
