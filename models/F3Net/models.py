...
class F3Net(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(
            4096 if self.mode == "Both" or self.mode == "Mix" else 2048, num_classes
        )
    ...
    def init_xcep_FAD(self):
        ...
        for i in range(4):
            self.FAD_xcep.conv1.weight.data[:, i * 3 : (i + 1) * 3, :, :] = (
                conv1_data / 4.0
            )
    ...
    def init_xcep_LFS(self):
        ...
        for i in range(int(self._LFS_M / 3)):
            self.LFS_xcep.conv1.weight.data[:, i * 3 : (i + 1) * 3, :, :] = (
                conv1_data / float(self._LFS_M / 3.0)
            )
...
def get_xcep_state_dict(
    pretrained_path="./models/F3Net/pretrained/xception-b5690688.pth",
):
    # load Xception
