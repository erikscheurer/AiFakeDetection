...
if __name__ == "__main__":
    ...
    # train
    model = Trainer(
        [*range(osenvs)] if osenvs > 0 else None, opt.mode, opt.pretrained_path
    )
    model.total_steps = 0
    epoch = 0
    ...
    while epoch < opt.max_epoch:
        ...
        for i, (inputs, labels) in enumerate(tqdm(dataloader)):
            ...
            tensorboard_logger.log_accuracy(
                model_output, model.label, model.total_steps, from_logits=True
            )
            tensorboard_logger.log_images(
                "train", model.input, model.label, model_output, model.total_steps
            )
            ...
            if model.total_steps % opt.model_save_freq == 0:
                ...
                model.save(
                    os.path.join(
                        opt.model_save_path,
                        f"{tensorboard_logger.start_time}_{epoch}_{model.total_steps}.pth",
                    )
                )
                logger.info(
                    f"saving the model at epoch {epoch}, iters {model.total_steps} at {opt.model_save_path}"
                )
        ...
        tensorboard_logger.add_scalar(
            "transfer_all_acc", overall_acc, epoch, writer="val"
        )
