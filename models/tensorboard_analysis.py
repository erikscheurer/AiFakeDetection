...
def plot_smoothed_runs(logdir, runs, tags, title=None, window=10, save_path=None, colors = 'bgrcmyk', labels=None):
    ...
    for i, tag in enumerate(tags):
        for j,run in enumerate(runs):
            ...
            axs[i].plot(data['step'], data['value'], alpha=0.1, color=colors[j], label='_nolegend_', linewidth=.1)
            # print metrics
            print(run)
...
#%%
logdir = 'output'
runs = ['F3Net/train/F3Net2024-06-02_00-29-20','CNNDetection/train/CNNDetection2024-06-04_08-46-28']
for run in runs:
