    ...
    for epoch in range(config.train.niter):
        ...
        for i, data in tqdm(
            ...
            if model.total_steps % config.train.save_latest_freq == 0:
                print(
                    f"saving the latest model (epoch {epoch}, total_steps {model.total_steps}), {logger.start_time} {target_generators}"
                )
                model.save_networks(
                    "latest", logger.start_time + str(target_generators)
                )

        if epoch % config.train.save_epoch_freq == 0:
            print(
                f"saving the model at the end of epoch {epoch}, iters {model.total_steps}, {logger.start_time} {target_generators}"
            )
