...
eps = .1 # max deviation from the original image
...
for i in range(10): # attack the first 10 images
    ...
    img1 = orig_image + mask*torch.tanh(shifted_weights) * eps
    ...
    model.eval()
    orig_output = model.model(img1)
    print(f"Original Prediction: {'AI' if torch.sigmoid(orig_output) < 0.5 else 'Nature'}")
    ...
    for i in tqdm(range(500)):
        ...
        img1 = orig_image + mask*torch.tanh(shifted_weights) * eps
        img1.clamp_(0, 1)
        output = model.model(img1)
        loss = loss_fn(output.squeeze(1), 1-label1)
        loss.backward()
        opt.step()
        print(torch.sigmoid(output).item(),f"{'AI' if torch.sigmoid(output) < 0.5 else 'Nature'}")
        if loss.item() < 1e-5:
            break
    ...
    axes[0].set_title(f"Original Pred: {'AI' if label1 == 0 else 'Nature'}: {torch.sigmoid(orig_output).item():.2f}")
    axes[0].axis('off')

    axes[1].imshow(img1[0].detach().permute(1, 2, 0))
    axes[1].set_title(f"Adversarial Pred: {'AI' if pred == 0 else 'Nature'}: {torch.sigmoid(output).item():.2f}")
    axes[1].axis('off')

    dev = (img1 - orig_image).detach()
    axes[2].imshow(dev[0].permute(1, 2, 0)+0.5)
    axes[2].set_title(f"Deviation: {torch.mean(torch.abs(dev)).item():.2f}")
    axes[2].axis('off')

    axes[3].imshow(((dev-dev.min())/(dev.max()-dev.min())).squeeze().permute(1, 2, 0))
