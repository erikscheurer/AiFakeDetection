        ...
        if model.total_steps % config.train.loss_freq == 0:
            for key in model_out:
                if key == 'output':
                    ...
                    if batch_size == 1:
                        logger.log_accuracy(output.squeeze(), model.label.item(), model.total_steps, from_logits=True)
                    else:
                        logger.log_accuracy(output.squeeze(), model.label.squeeze(), model.total_steps, from_logits=True)
                else:
