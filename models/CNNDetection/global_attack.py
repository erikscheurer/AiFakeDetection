...
eps = 0.5  # max deviation from the original image
...
imgs = (
    orig_image + mask * 0.5 * torch.tanh(torch.roll(weights, (h, w), dims=(2, 3))) * eps
)
...
print(
    f"Original Prediction: {'AI' if torch.sigmoid(orig_output)[0] < 0.5 else 'Nature'}"
)
...
for i in tqdm(range(200)):
    ...
    imgs = (
        image + mask * 0.5 * torch.tanh(torch.roll(weights, (h, w), dims=(2, 3))) * eps
    )
    imgs.clamp_(0, 1)
...
for i in range(images.shape[0]):
    ...
    axes[i, 0].set_title(
        f"Label: {'AI' if labels[0] == 0 else 'Nature'}: Pred: {'AI' if torch.sigmoid(orig_output)[i] < 0.5 else 'Nature'}: {torch.sigmoid(orig_output)[i].item():.2f}"
    )
    axes[i, 0].axis("off")

    axes[i, 1].imshow(image[i].detach().cpu().permute(1, 2, 0))
    axes[i, 1].set_title(
        f"Adversarial Pred: {'AI' if torch.sigmoid(output)[i] < 0.5 else 'Nature'}: {torch.sigmoid(output)[i].item():.2f}"
    )
    axes[i, 1].axis("off")
