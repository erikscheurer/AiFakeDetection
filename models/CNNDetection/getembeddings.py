...
if __name__ == "__main__":
    ...
    for generator in generators:
        ...
        model.load_networks(
            filename="output/ResNet_noPretraining/checkpoints/ResNet_noPretraining/2024-07-22_16-10-02['stable_diffusion_v_1_4']_model_epoch_40.pth"
        )
        ...
        for i, data in tqdm(
            enumerate(data_loader), total=len(data_loader), miniters=50
        ):
            model.total_steps += 1
            ...
            for j, (f, l, p) in enumerate(zip(features, label, pred)):
                os.makedirs(
                    f"{config.train.dataset.path.replace('GenImage','features_no_pretrain')}/{generator}/{'ai' if l==0 else 'real'}",
                    exist_ok=True,
                )
                np.save(
                    f"{config.train.dataset.path.replace('GenImage','features_no_pretrain')}/{generator}/{'ai' if l==0 else 'real'}/{i*batch_size+j}_{p[0]}.npy",
                    f,
                )
