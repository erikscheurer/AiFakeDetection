...
class Trainer(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isDANN:
            self.adversarial = AdversarialNetwork(
                in_feature=2048
            )  # 2048 is the output of the resnet50 before the fc layer
        ...
        if self.isTrain:
            ...
            if self.isDANN:
                self.optimizer.add_param_group(
                    {"params": self.adversarial.parameters(), "lr": opt.train.lr}
                )
    ...
    def optimize_parameters(self):
        ...
        if self.isDANN:
            ...
            indices = [
                i
                for i in range(len(self.generator_label))
                if self.generator_label[i] in self.source_gens
            ]
            source_outputs = self.output[indices]
        else:
            source_outputs = self.output
            indices = range(len(self.label))
        self.classify_loss = self.loss_fn(
            source_outputs.squeeze(1), self.label[indices]
        )
        self.loss = self.classify_loss

        if self.isDANN:
            self.DANN_loss = self.DANN_lossfn(
                self.DANN_output.squeeze(1)[self.generator_label != -1],
                self.generator_label[self.generator_label != -1].long(),
            )
        ...
        return {
            "output": self.output,
            "loss": self.loss,
            "DANN_loss": self.DANN_loss if self.isDANN else 0,
